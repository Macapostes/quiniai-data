...
# Ventana larga solo para H2H: la tabla y los últimos 5 siguen en HISTORY_SEASONS_BACK.
H2H_SEASONS_BACK = max(8, min(25, int(os.getenv("QUINIAI_H2H_SEASONS_BACK", "20"))))
H2H_LAST_N = max(10, min(40, int(os.getenv("QUINIAI_H2H_LAST_N", "30"))))
...
THESPORTSDB_SEARCH_EVENTS_URL = f"{_THESPORTSDB_BASE}/searchevents.php"
THESPORTSDB_EVENTS_NEXT_URL = f"{_THESPORTSDB_BASE}/eventsnext.php"
THESPORTSDB_EVENTS_LAST_URL = f"{_THESPORTSDB_BASE}/eventslast.php"
...
THESPORTSDB_LOOKUP_TABLE_URL = f"{_THESPORTSDB_BASE}/lookuptable.php"
...
LEAGUE_KEY_ALIASES = {
    ...
    "sportsdb_4354": "soccer_ukraine_premier_league",
    "sportsdb_4339": "soccer_turkey_super_league",
}
...
LEAGUE_THESPORTSDB_IDS = {
    ...
    "soccer_ukraine_premier_league": "4354",
...
def _temporadas_que_se_conservan() -> set[str]:
    ...
    """
    permitidas = set(_recent_season_codes(H2H_SEASONS_BACK))
    permitidas |= set(_sportsdb_recent_seasons(H2H_SEASONS_BACK))
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box} body{margin:0;font-family:Consolas,\"Cascadia Code\",monospace;background:radial-gradient(circle at top right,rgba(34,197,94,.12),transparent 28%),radial-gradient(circle at bottom left,rgba(56,189,248,.12),transparent 32%),var(--bg);color:var(--text)}
    ...
    .pestanas{display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap}
    .pestanas button{font:inherit;font-size:14px;font-weight:700;padding:9px 18px;border-radius:999px;
    ...
    .pestanas button[aria-selected="true"]{background:var(--accent);border-color:var(--accent);color:#04121f}
    .wrap.ancho{max-width:1460px}
    .marco-panel{border:1px solid var(--border);border-radius:18px;overflow:hidden;
      background:var(--panel);box-shadow:0 20px 60px rgba(0,0,0,.28)}
    .marco-panel iframe{display:block;width:100%;height:calc(100vh - 150px);min-height:560px;border:0}
    .nota-panel{color:var(--muted);font-size:12px;line-height:1.5;margin:10px 2px 0}
    .nota-panel a{color:var(--accent)}
...
<body>
  <div class="wrap" id="wrap">
    ...
    <section id="vista-usuarios" hidden>
      <div class="marco-panel">
        <iframe id="marco-usuarios" title="Panel de usuarios de QuiniAI"
                loading="lazy" referrerpolicy="no-referrer"></iframe>
      </div>
      <p class="nota-panel">
        El panel se sirve desde el backend, no desde esta pagina: la clave de
        administracion viaja solo a Railway y nunca pasa por GitHub Pages.
        Si prefieres verlo en su propia pestana,
        <a id="enlace-panel" target="_blank" rel="noopener">abrelo aqui</a>.
      </p>
    </section>

    <div id="vista-monitor">
    <div class="hero">
      <div class="top">
        <div>
  ...
  <script>
    // Pestanas: Monitor / Usuarios. El panel se sirve desde el backend, asi que
    // dentro del iframe sus llamadas son del mismo origen que la API: ni CORS,
    // ni la clave de administracion pasando por GitHub Pages.
    const PANEL_USUARIOS =
      "https://quiniela-backend-production-cb1a.up.railway.app/admin/panel";
    (function pestanas(){
      const bMonitor = document.getElementById("tab-monitor");
      const bUsuarios = document.getElementById("tab-usuarios");
      const vMonitor = document.getElementById("vista-monitor");
      const vUsuarios = document.getElementById("vista-usuarios");
      const marco = document.getElementById("marco-usuarios");
      const wrap = document.getElementById("wrap");
      document.getElementById("enlace-panel").href = PANEL_USUARIOS;
      function mostrar(cual){
        const usuarios = cual === "usuarios";
        vUsuarios.hidden = !usuarios;
        vMonitor.hidden = usuarios;
      ...
      bUsuarios.onclick = () => mostrar("usuarios");
      if (location.hash === "#usuarios") mostrar("usuarios");
    })();
...
def _team_similarity_score(left: str, right: str) -> float:
    ...
    if not left_norm or not right_norm:
        return 0.0
    reserve_markers = {
        "b", "castilla", "fortuna", "fabril", "mestalla", "promesas",
        "legends", "legend", "ii", "iii", "u17", "u18", "u19", "u20", "u21", "u23",
        "reserves", "reserve", "amateur", "oldboys",
    }
...
def _parece_equipo_satelite(nombre: str) -> bool:
    """Legends, filiales y juveniles no son el primer equipo."""
    tokens = set(_normalize_team_name(nombre).split())
    return bool(tokens & {
        "legends", "legend", "ii", "iii", "u17", "u18", "u19", "u20", "u21", "u23",
        "reserves", "reserve", "amateur", "oldboys",
    })
...
def _country_label_matches(expected: str, candidate: str) -> bool:
...
def _guess_country_hint(team_name: str, fallback: str | None = None) -> str | None:
    canonical = _canonical_team_name(team_name)
    normalized = _normalize_team_name(canonical)
    known = TEAM_COUNTRY_HINTS.get(normalized)
    if known:
        return known
    if fallback:
        return fallback
...
def fetch_the_sportsdb_team(team_name: str, country_hint: str | None = None) -> dict:
    ...
    if teams:
        ...
        for candidate in teams:
            ...
            if expected_country and candidate_country and not _country_label_matches(
                expected_country, candidate_country
            ):
                continue
            if _parece_equipo_satelite(candidate_name) and not _parece_equipo_satelite(team_name):
                continue
...
def _infer_league_key_from_sportsdb(*payloads: dict) -> str:
    league_id_map = {
        ...
        "4354": "soccer_ukraine_premier_league",
...
def _sportsdb_event_to_history_row(event: dict, season: str) -> dict:
    ...
    return {
        ...
        "idLeague": str(event.get("idLeague", "") or "").strip(),
        "strLeague": str(event.get("strLeague", "") or "").strip(),
...
def fetch_league_history(league_key: str, seasons_back: int | None = None) -> list[dict]:
    league_code = LEAGUE_FOOTBALL_DATA_CODES.get(league_key)
    if league_code:
        combined_rows = []
        for season_code in _recent_season_codes(seasons_back):
...
def _is_non_domestic_competition(league_key: object) -> bool:
    key = _canonical_league_key(league_key)
    if not key:
        return False
    if key in NON_DOMESTIC_LEAGUE_KEYS:
        return True
    lowered = key.lower()
    if any(
        token in lowered
        ...
        return True
    sportsdb_id = _sportsdb_league_id_for_key(key)
    if sportsdb_id in NON_DOMESTIC_SPORTSDB_IDS:
        return True
    if key.startswith("sportsdb_") and key.split("_", 1)[-1] in NON_DOMESTIC_SPORTSDB_IDS:
        return True
    return False


def _domestic_league_key_from_team_api(team_api: dict | None, match_league_key: str = "") -> str:
    """Liga doméstica del club (LaLiga, Ligue 1...), nunca la UCL del partido."""
    payload = team_api or {}
    inferred = _infer_league_key_from_sportsdb(payload)
    if inferred and not _is_non_domestic_competition(inferred):
        return inferred
    dynamic = _dynamic_league_key_from_sportsdb(payload)
    if dynamic and not _is_non_domestic_competition(dynamic):
        return dynamic
    match_key = _canonical_league_key(match_league_key)
    if match_key and not _is_non_domestic_competition(match_key):
        return match_key
    return ""


def _row_is_h2h(row: dict, home_team: str, away_team: str, threshold: float = 0.72) -> bool:
    row_home = str(row.get("HomeTeam") or "")
...
def _h2h_iso_date(row: dict) -> str:
    parsed = _row_parsed_date(row)
    if parsed:
...
def fetch_the_sportsdb_h2h_events(home_team: str, away_team: str) -> list[dict]:
    ...
    seen: set[tuple] = set()
    queries: list[str] = []
    for home in _h2h_search_names(home_team)[:3]:
        for away in _h2h_search_names(away_team)[:3]:
            for query in (f"{home} vs {away}", f"{away} vs {home}"):
    ...
    for query in queries[:8]:
        ...
        for event in events:
            ...
            row = _sportsdb_event_to_history_row(event, season)
            if not _row_is_h2h(row, home_team, away_team):
                continue
            key = (row.get("Date"), row.get("HomeTeam"), row.get("AwayTeam"), row.get("FTHG"), row.get("FTAG"))
            if key in seen:
                continue
            seen.add(key)
            row["League"] = str(event.get("strLeague") or "").strip()
            row["Source"] = "TheSportsDB-H2H"
            rows.append(row)
    if rows:
        _cache_set(THESPORTSDB_CACHE, cache_key, rows)
    return rows


def _ensure_league_history(histories: dict, league_key: str) -> list[dict]:
    key = _canonical_league_key(league_key)
    if not key:
        return []
    if key not in histories:
        histories[key] = fetch_league_history(key)
    return histories.get(key) or []


def _cup_history_needs_domestic_fill(match: dict) -> bool:
    if not _is_non_domestic_competition((match or {}).get("league")):
        return False
    history = (match or {}).get("history_context") or {}
    home = history.get("home") or {}
    away = history.get("away") or {}
    if not ((home.get("recent_all") or {}).get("form")):
        return True
    if not ((away.get("recent_all") or {}).get("form")):
        return True
    if not ((home.get("table") or {}).get("position")):
        return True
    if not ((away.get("table") or {}).get("position")):
        return True
    return False


def _history_row_is_domestic_league(row: dict, domestic_league_id: str = "") -> bool:
    inferred = _infer_league_key_from_sportsdb(row)
    if inferred and _is_non_domestic_competition(inferred):
        return False
    league_name = str(row.get("strLeague") or row.get("League") or "").lower()
    if any(token in league_name for token in ("champions", "europa", "conference", "friendly", "amistoso")):
        return False
    row_id = str(row.get("idLeague") or "").strip()
    if domestic_league_id and row_id and row_id != str(domestic_league_id):
        return False
    return True


def fetch_the_sportsdb_last_events(team_id: str) -> list[dict]:
    if not team_id:
        return []
    cache_key = f"sportsdb_last:v1:{team_id}"
    cached = _cache_get(HISTORY_CACHE, cache_key, HISTORY_CACHE_TTL_SECONDS)
    if cached:
        return list(cached)
    ...
    payload = events if isinstance(events, list) else []
    if payload:
        _cache_set(HISTORY_CACHE, cache_key, payload)
    return payload
...
def fetch_the_sportsdb_lookup_table(league_id: str, season_label: str) -> list[dict]:
    ...
    try:
        _frenar_sportsdb()
        data = _request_json(
            THESPORTSDB_LOOKUP_TABLE_URL,
            params={"l": league_id, "s": season_label},
            timeout=20,
        )
    except Exception as exc:
...
def _lookup_table_to_snapshot(rows: list[dict]) -> dict:
    table = {}
    for row in rows or []:
        team = str(row.get("strTeam") or row.get("name") or "").strip()
        if not team:
            continue
        played = _safe_int(row.get("intPlayed"), 0) or 0
        points = _safe_int(row.get("intPoints"), 0) or 0
        goals_for = _safe_int(row.get("intGoalsFor"), 0) or 0
        goals_against = _safe_int(row.get("intGoalsAgainst"), 0) or 0
        table[team] = {
            "team": team,
            "played": played,
            "points": points,
            ...
            "position": _safe_int(row.get("intRank"), None),
            "scope": "domestic",
...
def _sportsdb_domestic_fallback(
    team_name: str,
    team_api: dict | None,
    kickoff_dt: datetime | None,
    ...
    season_year = str(season.year if season.month >= 7 else season.year - 1)
    if team_id:
        for event in fetch_the_sportsdb_last_events(team_id):
            row = _sportsdb_event_to_history_row(event, str(event.get("strSeason") or season_year))
...
def _fill_side_from_sportsdb_if_empty(
    history: dict,
    team_name: str,
    team_api: dict | None,
    kickoff_dt: datetime | None,
    ...
    if extra:
        ...
        if not (filled.get("recent_all") or {}).get("form"):
            resolved = filled.get("resolved_name") or team_name
    ...
    if table_from_api:
        ...
        if api_row and api_row.get("position"):
            ...
            table.setdefault("league_name", _league_display_name(league_key, (team_api or {}).get("strLeague")))
            filled = dict(filled or {})
            filled["table"] = table
            filled.setdefault("resolved_name", api_row.get("team") or team_name)
            filled["league_key"] = league_key
            filled["league_name"] = table.get("league_name") or _league_display_name(league_key)
            filled["league_scope"] = "domestic"
    return filled or history


def _team_history_with_scope(
    rows: list[dict],
    team_name: str,
    kickoff_dt: datetime | None,
    league_key: str,
    ...
    history["league_name"] = _league_display_name(league_key)
    history["league_scope"] = scope
    return history
...
def _resolve_domestic_histories_and_h2h(
    *,
    ...
    league_key: str,
    histories: dict,
    home_team_api: dict | None,
    away_team_api: dict | None,
    kickoff_dt: datetime | None,
    home_ctx_kwargs: dict | None = None,
    ...
    away_scope = (
        "domestic"
        if away_domestic and not _is_non_domestic_competition(away_domestic)
        else "competition"
    )
    home_history = _team_history_with_scope(
        home_rows,
        home_team,
        kickoff_dt,
        home_domestic or league_key,
        home_scope,
        home_ctx_kwargs,
    )
    away_history = _team_history_with_scope(
        away_rows,
        away_team,
        kickoff_dt,
        away_domestic or league_key,
        away_scope,
        away_ctx_kwargs,
    )
    if _is_non_domestic_competition(league_key):
        home_history = _fill_side_from_sportsdb_if_empty(
            home_history or {},
            home_team,
            home_team_api,
            kickoff_dt,
        ...
        away_history = _fill_side_from_sportsdb_if_empty(
            away_history or {},
            away_team,
            away_team_api,
            kickoff_dt,
    ...
    h2h_rows: list[dict] = []
    h2h_keys = []
    for key in (league_key, home_domestic, away_domestic):
    ...
    if kickoff_dt:
        kd = kickoff_dt
        if kd.tzinfo is not None:
            kd = kd.astimezone(timezone.utc).replace(tzinfo=None)
        kickoff_day = kd.date()
        sin_hoy = []
        for row in completed:
            parsed = _row_parsed_date(row)
            if parsed is not None:
                parsed_day = parsed.astimezone(timezone.utc).date() if parsed.tzinfo else parsed.date()
                if parsed_day == kickoff_day and _row_is_h2h(row, home_team, away_team):
                    continue
            sin_hoy.append(row)
        completed = sin_hoy
    home_resolved = home_history.get("resolved_name", home_team)
    away_resolved = away_history.get("resolved_name", away_team)
...
def _head_to_head_metrics(rows: list[dict], home_team: str, away_team: str, last_n: int = 10) -> dict:
    ...
    for row in meetings:
        result = row.get("FTR", "")
        row_home = str(row.get("HomeTeam") or "")
        current_home_was_home = _team_similarity_score(row_home, home_team) >= 0.72
        if result == "D":
            draws += 1
        elif (result == "H" and current_home_was_home) or (result == "A" and not current_home_was_home):
            home_team_wins += 1
        else:
            away_team_wins += 1
        recent_matches.append(
            {
                "date": _h2h_iso_date(row),
    ...
    last_dt = _parse_match_date(str(meetings[-1].get("Date", "")).strip()) or _row_parsed_date(meetings[-1])
    return {
        "meetings": len(meetings),
        "home_team_wins": home_team_wins,
        "away_team_wins": away_team_wins,
        "wins_local": home_team_wins,
        "wins_visit": away_team_wins,
        "draws": draws,
        "years_span": (
            ((last_dt.year if last_dt else datetime.now(timezone.utc).year)
             - (first_dt.year if first_dt else datetime.now(timezone.utc).year))
            if meetings
            else 0
        ),
...
def _bootstrap_quiniela_placeholder(
    ...
    home_history, away_history, h2h_history = _resolve_domestic_histories_and_h2h(
        ...
        away_team=nombre_hist_visitante,
        league_key=league_key,
        histories=histories,
    ...
    if not h2h_history:
        h2h_history = _head_to_head_metrics(
            ...
            home_history.get("resolved_name", home_team),
            away_history.get("resolved_name", away_team),
            last_n=H2H_LAST_N,
        )
        ...
        for jornada in quiniela_jornadas:
            for match in jornada.get("matches", []):
                ...
                needs_bootstrap = (
                    ...
                    or _cup_history_needs_domestic_fill(match)
                    or
